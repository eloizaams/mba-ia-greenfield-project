...
function asAuthBody(res: request.Response): AuthApiResponseBody {
  return res.body as AuthApiResponseBody;
}
...
describe('Auth (e2e)', () => {
  ...
  async function registerConfirmAndLogin(
    ...
    const res = await request(app.getHttpServer())
      ...
      .send({ email, password });
    return {
      access_token: asAuthBody(res).access_token!,
      refresh_token: asAuthBody(res).refresh_token!,
    };
  }
  ...
  describe('POST /auth/register', () => {
    it('returns 201 with { id, email } on valid registration', async () => {
      ...
      expect(asAuthBody(res).id).toBeDefined();
      expect(asAuthBody(res).email).toBe('user@example.com');
    });
    ...
    it('returns 409 with EMAIL_ALREADY_EXISTS on duplicate email', async () => {
      ...
      expect(asAuthBody(res).error).toBe('EMAIL_ALREADY_EXISTS');
    });
    ...
    it('returns 400 with VALIDATION_ERROR on missing email', async () => {
      ...
      expect(asAuthBody(res).error).toBe('VALIDATION_ERROR');
    });
  ...
  describe('GET /auth/confirm-email', () => {
    ...
    it('returns 401 with INVALID_TOKEN on an already-used token', async () => {
      ...
      expect(asAuthBody(res).error).toBe('INVALID_TOKEN');
    });
    ...
    it('returns 401 with TOKEN_EXPIRED on an expired token', async () => {
      ...
      expect(asAuthBody(res).error).toBe('TOKEN_EXPIRED');
    });
  ...
  describe('JWT Guard', () => {
    ...
    it('returns 200 on GET /auth/me with a valid access token', async () => {
      ...
      expect(asAuthBody(res).sub).toBeDefined();
      expect(asAuthBody(res).email).toBe('me@example.com');
    });
  ...
  describe('POST /auth/login', () => {
    ...
    it('returns 200 with access_token and refresh_token on valid credentials', async () => {
      ...
      expect(asAuthBody(res).access_token).toBeDefined();
      expect(asAuthBody(res).refresh_token).toBeDefined();
      expect(typeof asAuthBody(res).access_token).toBe('string');
      expect(typeof asAuthBody(res).refresh_token).toBe('string');
    });
    ...
    it('returns 401 with INVALID_CREDENTIALS on wrong password', async () => {
      ...
      expect(asAuthBody(res).error).toBe('INVALID_CREDENTIALS');
    });
    ...
    it('returns 403 with EMAIL_NOT_CONFIRMED when user is not confirmed', async () => {
      ...
      expect(asAuthBody(res).error).toBe('EMAIL_NOT_CONFIRMED');
    });
  ...
  describe('POST /auth/refresh', () => {
    it('returns 200 with new access_token and refresh_token on valid refresh token', async () => {
      ...
      expect(asAuthBody(res).access_token).toBeDefined();
      expect(asAuthBody(res).refresh_token).toBeDefined();
      expect(asAuthBody(res).refresh_token).not.toBe(refresh_token);
    });
    ...
    it('returns 401 with TOKEN_REUSE_DETECTED when reuse is beyond grace period', async () => {
      ...
      expect(asAuthBody(res).error).toBe('TOKEN_REUSE_DETECTED');
  ...
  describe('POST /auth/reset-password', () => {
    it('returns 204 with a valid token and updates the password', async () => {
      ...
      const loginRes = await request(app.getHttpServer())
        ...
        .expect(200);
      expect(asAuthBody(loginRes).access_token).toBeDefined();
    });
