...
import { CreateVideos1783097595548 } from './migrations/1783097595548-CreateVideos';
...
// Ordem de drop respeita as FKs (videos → channels → users).
const DROP_ORDER = [
  'videos',
  'refresh_tokens',
  'verification_tokens',
...
// DROP TABLE não remove tipos enum — sem isso o re-run das migrations falha
// em CREATE TYPE com "type already exists" e deixa a tabela migrations
// inconsistente com o schema.
...
describe('Database migrations (integration)', () => {
  ...
  afterAll(async () => {
    ...
    // missing. Re-apply so the shared DB is fully migrated when subsequent
    // suites run.
