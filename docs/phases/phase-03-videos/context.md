...
sources_mtime:
  docs/project-plan.md: "2026-07-02T15:02:19-03:00"
  ...
  docs/phases/phase-01-configuracao-base/context.md: "2026-07-02T15:02:19-03:00"
  docs/phases/phase-02-auth/context.md: "2026-07-02T15:02:19-03:00"
  docs/phases/phase-02-auth-frontend/context.md: "2026-07-02T15:02:19-03:00"
  .claude/skills/testing-guide-nestjs-project/SKILL.md: "2026-07-02T15:02:19-03:00"
---
...
**Affected subprojects:** `nestjs-project/` (all capabilities — storage service, upload endpoints, queue, worker, FFmpeg processing, unique URL generation, streaming/download delivery). `next-frontend/` is touched only through two Cross-layer contracts (TD-02/TD-08 upload protocol, TD-07 streaming/download delivery) — no screen capability bullet exists in this phase.
...
|-----|--------|-------|-------|--------|----------|-----------|
| phase-03-videos/TD-01 | phase | Backend | Object Storage Backend (dev/prod S3-compatible store) | decided | A (MinIO + AWS SDK v3) | — |
| phase-03-videos/TD-02 | phase | Cross-layer | Upload Protocol for 10GB Resumable Uploads | superseded-by phase-03-videos/TD-08 | — | — |
| phase-03-videos/TD-03 | phase | Backend | Background Job Queue Technology | decided | A (BullMQ + Redis via `@nestjs/bullmq`) | — |
| phase-03-videos/TD-04 | phase | Backend | Video Worker Process Topology | decided | B (separate worker entrypoint in `nestjs-project`, own Compose service) | — |
| phase-03-videos/TD-05 | phase | Backend | FFmpeg/ffprobe Integration Strategy | decided | A (system FFmpeg in worker image + direct `execFile`) | — |
| phase-03-videos/TD-06 | phase | Backend | Public Video ID / Unique URL Generation | decided | A (`nanoid` custom alphabet + unique constraint) | — |
| phase-03-videos/TD-07 | phase | Cross-layer | Streaming and Download Delivery | decided | A (presigned GET direct from storage) | — |
| phase-03-videos/TD-08 | phase | Cross-layer | Upload Protocol — Reconsidered under Strict-BFF Constraint | decided | A (presigned S3 multipart upload, browser-to-storage direct via Uppy) | @uppy/core, @uppy/aws-s3 |

_Note: TD-08 resolved `ICC-1` (raised by `/plan-validate 03`) by reconsidering TD-02's network topology. TD-02 is now superseded by TD-08 — the `superseded-by` marker was injected on TD-02's heading during `/plan-resolve 03` (2026-07-02)._
...
| Upload de vídeos com suporte a arquivos de até 10GB sem impacto na performance | phase-03-videos/TD-08 |
...
**Recommendation:** it is the literal store named in the architecture diagram, the only option with battle-tested multipart/presigned behavior under `@tus/s3-store` (TD-02) and presigned Range reads (TD-07). The 2025 Community Edition console gutting does not affect programmatic S3 API usage, which is all this project needs; bucket provisioning is scriptable via `mc` in the Compose setup. Per Docker networking convention, the service host is the Compose service name (e.g., `storage`), never `localhost`.

**Libraries:** —

### phase-03-videos/TD-03

**Recommendation:** it is the framework-idiomatic choice with first-party NestJS documentation, and its retry/backoff/progress model matches long-running FFmpeg jobs. pg-boss is honestly viable at this project's scale and avoids Redis, but the official integration, ecosystem examples, and the architecture's explicit "Message Queue" container tip the balance to BullMQ; the transactional-enqueue gap is handled by enqueueing only after commit.

**Libraries:** —

### phase-03-videos/TD-04

**Recommendation:** it delivers the process isolation the performance requirement and the C4 diagram demand, at the cost of one extra bootstrap file instead of a whole new project. Option C's boundary only pays off if the worker ever leaves TypeScript/Nest, which nothing on the roadmap suggests.
...
**Recommendation:** the only option that is simultaneously short (plan requirement) and non-enumerable (unlisted-link requirement inherited by Fases 04/05), at the cost of one column. Option C is disqualified by enumerability; Option B by URL length.

**Libraries:** —

### phase-03-videos/TD-07

**Recommendation:** it is the architecture the C4 diagram already draws, keeps 10GB byte streams off the Node process, and gets correct Range semantics for free from the storage server; the browser-reachable-endpoint caveat is a one-time Compose/config task. Download rides the same mechanism via `response-content-disposition`. HLS is deliberately out of scope for this phase.

**Libraries:** —

### phase-03-videos/TD-08

**Recommendation:** it is the only option that resolves the Strict-BFF conflict by construction rather than by exception, reusing the exact pattern the architecture already accepts for TD-07 and already anticipates in its env-key model. It also removes 10GB of byte traffic from both Next.js and NestJS entirely, which is a strictly better fit for the phase's performance requirement than even TD-02's original tus design. Option B is technically feasible in this self-hosted project but adds a full relay layer for no functional gain over Option A. Option C keeps tus's semantics but only shrinks the Strict-BFF exception instead of eliminating it.
...
_(from testing-guide-nestjs-project — no screen/UI work in this phase; `next-frontend/` has no artifacts to test here, only a future consumer of the Cross-layer contracts fixed by TD-02/TD-07/TD-08)_
...
| Service with side-effect dep (storage upload, thumbnail extraction) | Integration: real capture/adapter (e.g. MinIO test bucket) |
...
_Note (worth testing, per guide §2): service-to-external-system contracts (storage uploads, queue publishing), module DI wiring for every module with configured imports (`BullModule.registerQueue()`, S3 client provider), and race conditions (concurrent uploads) are explicitly in-scope test categories for this phase's artifacts._

### next-frontend

_Deferred subproject — no implementation artifacts in this phase (Cross-layer contracts TD-02/TD-07/TD-08 are fixed here but consumed by a future phase); testing requirements will be defined when that phase implements the consuming code._
